...
def page2():
    ...
    if "pressure_df" not in st.session_state:
        st.session_state.pressure_df = pd.DataFrame(
            columns=["RubyLine", "Press", "DAC"]
        )
    ...
    with left_column:
        ...
        if st.button("Add Pressure to DataFrame", key="add_pressure"):
            new_row = pd.DataFrame(
                {"RubyLine": [rubypos], "Press": [pressure], "DAC": [0]}
            )
            st.session_state.pressure_df = pd.concat(
                [st.session_state.pressure_df, new_row], ignore_index=True
            )
            st.success("Pressure value added to DataFrame")

        # Display the editable DataFrame
        pressure_df = st.data_editor(
            st.session_state.pressure_df, key="pressure_editor"
        )
    ...
    with right_column:
        ...
        if st.button("Add Temperature to DataFrame", key="add_temp"):
            new_row = pd.DataFrame(
                {"linepos": [line4temp], "temperature": [temperature]}
            )
            st.session_state.temp_df = pd.concat(
                [st.session_state.temp_df, new_row], ignore_index=True
            )
...
def graphandfitruby():
    ...
    if uploaded_file is not None:
        ...
        # Store in session state for Fit Ruby page
        st.session_state.data = data
        st.session_state.file_loaded = True
        st.session_state.ruby_filename = uploaded_file.name
        ...
        fig.update_layout(
            ...
            xaxis_title="nm",
            yaxis_title="Counts",
            font=dict(family="Helvetica", size=28, color="RebeccaPurple"),
        )
