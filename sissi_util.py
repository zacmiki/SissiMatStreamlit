...
def allOpusFiles(dirName):
    ...
    opus_files = [
        ...
        if os.path.getsize(file) > 0 and opusFC.isOpusFile(file)
    ]
    ...
    for item in range(len(dbs)):
        if (dbs[item][0]) != "SIFG":
            ...
            if graphornot == None:
                ...
                ftir1.minorticks_on()
                ftir1.set(
                    xlabel="Wavenumbers (cm-1)", ylabel="Intensity", title=labella
                )
                ftir1.plot(
                    data.x, data.y, label=suffix, linewidth=0.5
                )  # Plot IR Transformed spectrum.
                ftir1.legend()

                ftir1.grid(which="both", axis="x", lw=0.2)
                ftir1.grid(which="major", axis="y", linewidth=0.2)
                plt.show()
            ...
            if paramornot == None:
                ...
                print(
                    f"Aperture =\t{data.parameters['APT']} \nBSplitter = \t{data.parameters['BMS']}"
                )
                print(
                    f"Source = \t{data.parameters['SRC']} \nDetector = \t{data.parameters['DTC']}"
                )
                print(
                    f"Frequency = \t{data.parameters['VEL']} kHz\nChannel = \t{data.parameters['CHN']}"
                )
                print(f"Resol = \t{data.parameters['RES']} cm-1")
                print(
                    f"Data from:\t{data.parameters['LXV']} to {data.parameters['FXV']} cm-1"
                )
...
def averageFiles(fileDir, fileToAverage, nfiles):
    ...
    plt.rcParams["figure.figsize"] = [
        10,
        7,
    ]  # sets the default window size of inline plots
